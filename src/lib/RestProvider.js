...
const RestProvider = (firebaseConfig = {}, options = {}) => {
  ...
  const lsGetList = legiscanMethods.getList;
  const lsGetOne = legiscanMethods.getOne;
  const lsCreate = legiscanMethods.create;
  ...
  trackedResources.map((resource, index) => {
    ...
    resourcesPaths[name] = path || firebaseRootPath + name;
    resourcesData[name] = {};
  });
  ...
  return async (type, resourceName, params) => {
    if (resourceName.startsWith('ls')) {
      let result = null;
      switch (type) {
        case GET_LIST:
          result = await lsGetList(resourceName, params);
          return result;
        case GET_ONE:
          result = await lsGetOne(resourceName, params);
          return result;
        case UPDATE:
        case CREATE:
          result = await lsCreate(resourceName, params);
          return result;
      }
    }

    const resourcePath = firebaseRootPath + resourceName;
    await resourcesStatus[resourceName];
    let result = null;
    switch (type) {
      case GET_LIST:
        result = await getList(params, resourceName, resourcesData[resourceName], resourcePath);
        return result;
      case GET_MANY:
        result = await getMany(params, resourceName, resourcesData[resourceName], resourcePath);
        console.log('GET_MANY');
        return result;

      case GET_MANY_REFERENCE:
        console.log('GET_MANY_REFERENCE');
        result = await getManyReference(params, resourceName, resourcesData[resourceName], resourcePath);
        return result;

      case GET_ONE:
        result = await getOne(params, resourceName, resourcesData[resourceName], resourcePath);
        return result;
