...
const getOne = async (params, resourceName, resourceData, resourcePath) => {
  if (params.id && params.id.toString()) {
    let result = await firebase.firestore().collection(resourcePath).doc(params.id.toString()).get();
...
const getList = async (params, resourceName, resourceData, resourcePath) => {
  if (params.pagination) {
    let values = [];
    let snapshots = await firebase.firestore().collection(resourcePath).get();
...
const getMany = async (params, resourceName, resourceData, resourcePath) => {
  let data = [];
  /* eslint-disable no-await-in-loop */
  for (const id of params.ids) {
    let { data: item } = await getOne({ id }, resourceName, resourceData, resourcePath);
    data.push(item);
  }
  return { data };
};

const getManyReference = async (params, resourceName, resourceData, resourcePath) => {
  if (params.target) {
    if (!params.filter) params.filter = {};
    params.filter[params.target] = params.id.toString();
    let { data, total } = await getList(params, resourceName, resourceData, resourcePath);
    return { data, total };
  } else {
    throw new Error('Error processing request');
  }
