...
const BillTimelineEditForm = ({ basePath, record, version, redirect, currentTrackerItem }) => {
  ...
  return (
    <Form
      ...
      render={formProps => (
        <Box style={{ width: '95%', margin: '0!important', padding: '0!important' }}>
          <Box display="flex">
            <Box flex={1}>
          ...
          <Box display="flex">
            <TextInput style={{ width: '100%', marginRight: '12px' }} source="title" />
          </Box>
          <Box display="flex" style={{ marginRight: '12px' }}>
            <RichTextInput style={{ width: '100%' }} source="description" stripTags={false} label="" addLabel={false} />
          </Box>
          <Box display="flex">
