...
import { SaveButton, useCreate, useRedirect, useNotify, sanitizeEmptyValues } from 'react-admin';
...
const SubmitCreateButton = ({ basePath, redirect, formProps, tlRecord }) => {
    ...
    create(
      {
        ...
        payload: {
          data: {
            ...
            prev_body_id: formValues.prev_body_id,
            prev_tracker_step_id: formValues.prev_tracker_step_id,
            prev_committee_id: formValues.prev_committee_id,
  ...
  }, [formState.valid, formState.values, create, notify, redirectTo, redirect, basePath]);
...
const BillTrackerTimelineCreateForm = ({ basePath, record, version, redirect, currentTrackerItem, tlRecord }) => {
        ...
        <Grid className={classes.formGrid} container spacing={3}>
          <Grid container xs={12} spacing={2}>
            <Grid item hidden xs={12}>
              <Field
                defaultValue={currentTrackerItem.order + 1}
                label="Order"
                name="order"
                component={TextField}
                type="number"
              ...
              />
              <Field
                defaultValue={currentTrackerItem.body_id}
                label="Previous body_id"
                name="prev_body_id"
                component={TextField}
                type="number"
                fullWidth
              />
              <Field
                defaultValue={currentTrackerItem.tracker_step_id}
                label="Previous tracker_step_id"
                name="prev_tracker_step_id"
                component={TextField}
                type="text"
                fullWidth
              />
              <Field
                defaultValue={currentTrackerItem.committee_id}
                label="Previous committee_id"
                name="prev_committee_id"
                component={TextField}
                type="number"
                fullWidth
              />
              <Field label="Tracker Path ID" name="tracker_path_id" component={TextField} type="text" fullWidth />
            </Grid>
            <Grid item xs={6}>
              <Grid container>
          ...
          <Grid className={classes.currentStepGrid} container spacing={2}>
            <Grid style={{ marginLeft: '12px' }} item xs={3}>
              ...
              <DisplayReference record={currentTrackerItem} source="body_id" resource="bodies" display="body_name" />
            </Grid>
            <Grid item xs={5}>
          ...
          <Condition when="action" is="Move">
            <Grid className={classes.moveBoxGrid} style={{ marginLeft: '12px' }} container spacing={2}>
              <Grid container xs={2} spacing={3}>
                ...
                <Grid item xs={9}>
                  <Field defaultValue={currentTrackerItem.body_id} name="body_id" component="select" fullWidth>
                    {props => (
                      <SelectWithLookup
                        onChange={props.input.onChange}
                        name={props.input.name}
          ...
          <Grid container style={{ marginTop: '12px' }} xs={11}>
            ...
            <Grid item xs={9}>
              <Field label="Callout Message" name="callout_message" type="text" component={TextField} fullWidth />
            </Grid>
          </Grid>
